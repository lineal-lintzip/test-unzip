...
def validate_job_input(job: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if source.account != destination.account:
        logging.info(
            "Cross-account copy detected: %s -> %s",
            source.account,
            destination.account,
        )
...
def upload_extracted_files(
    ...
    for item in files:
        ...
        remote_path = "/".join(
            part for part in [destination.normalized_path, rel_path] if part
        )
...
async def terminate_job(
    ...
    return func.HttpResponse(
        json.dumps({"instanceId": instance_id, "message": reason}),
        status_code=202,
        mimetype="application/json",
    )
...
async def purge_job_history(
    ...
    return func.HttpResponse(
        json.dumps({"instanceId": instance_id, "message": "History purged."}),
        status_code=200,
        mimetype="application/json",
    )
...
def unzip_orchestrator(context: df.DurableOrchestrationContext):
    ...
    extracted = yield context.call_activity(
        "extract_zip_activity",
        {
            "validated": validated,
            "download": download,
        },
    ...
    upload = yield context.call_activity(
        "upload_files_activity",
        {
            "validated": validated,
            "extracted": extracted,
        },
